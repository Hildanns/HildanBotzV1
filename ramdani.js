...
module.exports = Ramdani = async (Ramdani, mek) => {
	try {
        ...
        const listmsg = (from, title, desc, list) => { // ngeread nya pake rowsId, jadi command nya ga keliatan
            let po = Ramdani.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "Pilih Disini","footerText": "👑𝐻𝑖𝑙𝑑𝑎𝑛𝑛","listType": "SINGLE_SELECT","sections": list}}, {})
...
"externalAdReply": {
      ...
      const demoteAdmin = async function(to, target=[]){
          ...
          if (!packname) packname = 'Dibuat Oleh'; if (!author) author = '© ʜɪʟᴅᴀɴɴ';author = author.replace(/[^a-zA-Z0-9]/g, '');	
...
danzrun = process.uptime() 
           Ramdani.setStatus(`${namabot} CODE ${(danzrun)} © 𝐵𝑦 𝐻𝑖𝑙𝑑𝑎𝑛𝑛`).catch((_)=>_); //DI UBAH YATIMM
...
`
        ...
        if (isBanned) return freply(mess.banned)
        freply(mess.wait)
        allmenu =`*Halo Kak👋 ${pushname} ${ucapanWaktu}*
━━━━━ *INFO USER* ━━━━━
❏ NAMA : *${pushname}*
...
> 532 ${prefix}nsfw_avatar`
               ...
               buttonsMessage = {
               contentText: `${allmenu}`,
               footerText: '© ʙʏ 𝙷𝚒𝚕𝚍𝚊𝚗𝚗', imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
...
if (isBanned) return freply(mess.banned)
freply(`━━━━━ *SCRIPT* ━━━━━
❏ LINK VIA GITHUB : nonono
❏ BASE ORI : Hildann
❏ TYPE SC : nodejs
❏ DI BUAT PADA : 𝚂𝚎𝚗, 4, 𝚊𝚙𝚛𝚒𝚕, 2022
*━━━━━━━━━━━━━━━━━━━━*`)
break
case 'alllmenu2':
if (!isRegist) return freply(mess.regist)
...
break
      ...
      qris =`*SCAN QRIS NYA OM, TEEF SEIHKLASNYA🗿*`
               ...
               buttonsMessage = {
               contentText: `${qris}`,
               footerText: '© 𝙱𝚢 𝙷𝚒𝚕𝚍𝚊𝚗𝚗', imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
...
// SOSMET          
      ...
      if (!isRegist) return freply(mess.regist)
      if (isBanned) return freply(mess.banned)
             freply(`Follow Instagram Owner Ya : https://www.instagram.com/hlfa_hildan`)
             break
//casee nya
case 'grupbot':
case 'groupbot':
...
Ramdani.sendMessage(_.jid, 
      ...
      case 'broadcastgroup': 
             ...
             for (let _ of anu) {
Ramdani.sendMessage(_.jid, 
			{"contentText": `*「 SIARAN HILDAN BOTZ」*\n*Izin Siaran Ngap:v*\n*Pesan :* ${body.slice(4)}`,
			"footerText": '© 𝐵𝑦 𝐻𝑖𝑙𝑑𝑎𝑛𝑛',
			"buttons": [
			{"buttonId": `${prefix}menu`,
			"buttonText": {"displayText": "MENU"
			},"type": "RESPONSE"}
