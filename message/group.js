		...
		try {
            ...
            Ramdani.sendMessage(anu.jid, 'Halo! Terima Kasih sudah Mengundangku, Jika ingin Menggunakan Bot Ketik ${prefix}menu', 'conversation')
             ...
             if (!welkom.includes(anu.jid)) return
                ...
                buttons = [{buttonId: `#`,buttonText:{displayText: 'Haloo :D'},type:1}]
                imageMsg = (await Ramdani.prepareMessageMedia((buff), 'imageMessage', {thumbnail: buff})).imageMessage
                buttonsMessage = { contentText: `${teks}`, footerText: 'Baca deskripsi dulu ya, jangan langsung masuk aja', imageMessage: imageMsg, buttons: buttons, headerType: 4 }
            ...
            if (!welkom.includes(anu.jid)) return
                ...
                buttons = [{buttonId: `.`,buttonText:{displayText: 'Bye Bro :)'},type:1}]
                imageMsg = (await Ramdani.prepareMessageMedia((buff), 'imageMessage', {thumbnail: buff})).imageMessage
                buttonsMessage = { contentText: `${out}`, footerText: 'Dadaa :)', imageMessage: imageMsg, buttons: buttons, headerType: 4 }
                prep = await Ramdani.prepareMessageFromContent(mdata.id,{buttonsMessage},{})
                Ramdani.relayWAMessage(prep)
            }
		} catch (e) {
